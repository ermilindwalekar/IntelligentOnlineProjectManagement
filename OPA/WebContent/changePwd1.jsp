<%-- 
    Document   : changePwd1
    Created on : Apr 18, 2015, 6:32:30 PM
    Author     : user
--%>
    ...
    <body>
       <%
		...
		Boolean pwd=retrieve(uid,CP);
			...
			RequestDispatcher rd=request.getRequestDispatcher("s_changepassword.jsp");
...
<%!
    private Boolean retrieve(String uid,String CP) {
		...
		Connection con=DriverManager.getConnection("jdbc:mysql://localhost:3306/mysql","root","root");
            ...
            String query="SELECT password from login where userid='"+uid+"'";
            rs = st.executeQuery(query);
             if (rs.next()) {
                passwd = rs.getString(1).trim();
            }
      ...
      PreparedStatement pst=con.prepareStatement("select password from login where userid=?");
	pst.setString(1,uid);
